...
if(mysql_num_rows($res)==0){
	...
	$list_items='<option value="0">Festivos Genericos</option>';
	...
	if( ($_REQUEST["id_item"]) && (!in_array($_REQUEST["id_item"],$user_items)) ){
		...
		if(!isset($_REQUEST["id_item"])) $_REQUEST["id_item"]=0;	
		$the_file=AC_INLCUDES_ROOT."cal.inc.php";
		if(!file_exists($the_file)) die("<b>".$the_file."</b> not found");
		else		require_once($the_file);
		
		$xtra_js_files.='<script type="text/javascript" src="aplicacion/php/admin/calendario/ac-admin/js/mootools-cal-adminj.js"></script>
		';
		...
		$free_fran='';
		if(ID_ITEM!=0)
			$free_fran='<option class="no_generico_fran" value="free">'.$lang["available"].'</option>';
		$contents.='
		<form id="form_items_fran">
		<table>
			<tr>
				<input type="hidden" name="page" value="'.ADMIN_PAGE.'">
				<td class="side" style="width:100px;">'.$lang["item_to_show"].'</td>
				<td>
					<select name="id_item" class="select" onchange="this.form.submit();" id="id_item_sel">
						'.$list_items.'
					</select>
					<!--<input type="submit" value="'.$lang["bt_change_item"].'" style="">-->
				</td>
				<td>&nbsp;&nbsp;</td>
				<td><!--<input type="button" value="'.$lang["bt_add_item"].'" style="" onclick="document.location.href=\'edit-calendario/?page=items&action=new\'">--></td>
			</tr>
			<tr>
				...
				<td>
					<select id="id_predefined_state"  class="select" >
						...
						'.$sel_list_states.$free_fran.'
						<!--<option class="no_generico_fran" value="free">'.$lang["available"].'</option>-->
					</select>
				</td>
			</tr>
		</table>
