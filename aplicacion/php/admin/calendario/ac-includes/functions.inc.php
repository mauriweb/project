...
function draw_cal($id_item,$month,$year,$manage_type="avail"){
	...
	if(!isset($list_day_titles))$list_day_titles='';
	if(!isset($show_week_num))$show_week_num=false;
	...
	$row_counter=0;
	if(!isset($list_days))$list_days='';
	if(!isset($show_week_num))$show_week_num='';
	if($first_week_day!=7){
	...
	while($row=mysql_fetch_assoc($res)){
		if($row["id_item"]!='0'){
			...
			if(!isset($booked_days[$row["the_date"]])){
				$booked_days[$row["the_date"]]=array("class"=>'festivo', "state"=>$row["the_state"]);
			}else{
				$booked_days[$row["the_date"]]['class'].=' festivo ';
			}
		}
	}
