<?php 
	/*CANONICAL*/
	$smarty->assign('pagina','/informes-material');
	/*SEO*/
	...
	if(isset($_POST['aplicar']) && $ids!='' && trim($_POST['num-factura'])!=''){
	    $res=$db->GetAll("select idPedido from partes_material where id IN ($ids) group by idPedido");
		$count=count($res);
		if($count==1){
			$ids_=explode(',', $ids);
			foreach($ids_ as $id){
		    	$db->execute('update partes_material set facturado="s", numFactura="'.sanitize($_POST['num-factura'], SQL).'" where 
	...
	if(count($res)>0){
	  foreach($res as $re){
	      ...
	      $partes.='<tr class="odd gradeX">
                                        ...
                                        <td >'.$re['numFactura'].'</td>
                                        <td><a href="ver-partes-de-material-informe/?idMat='.$re['id'].'" class="btn default btn-xs blue"><i class="fa fa-external-link"></i> Ver </a></td>
	...
	if($ids!='' && isset($_POST['excel'])){
		...
		pm.fecha fechapm, pm.numFactura, pm.id idParte, pm.visado, pm.firmado, c.empresa cliente, p.codigo proy, pp.numero, e.nombre resp, pmd.* from 
		  ...
		  empleados e, 
		  partes_material_detalle pmd, 
		  partes_material_codigos pmc 
		  WHERE 
		  pm.id IN ($ids) AND 
		  pm.idPedido=pp.id AND 
		  pp.idProyecto=p.id AND 
		  pp.idCliente=c.id AND 
		  pmd.idPartesMaterial=pm.id AND
		  pmd.codigo=pmc.id AND 
	...
	if(count($excel)>0){
		...
		$camposDetalle=array('FECHA DETALLE', 'CODIGO', 'FIRMADO', 'VISADO', 'DIMENSIONES', 'TOTAL FACTURABLE', 'TOTAL NO FACTURABLE', 'PEIDO POR');
		$datosDetalle=array('fecha', 'codigo', 'firmado', 'visado', 'dimensiones', 'totFact', 'totNoFact', 'pedidoPor');
		...
		foreach($excel as $re){
			...
			foreach($datos as $key=>$val){
				...
				if($val=='codigo'){
				    $sql="select codigo from partes_material_codigos where id='".$valor."'";
					$res=$db->GetRow($sql);
					$valor=$res['codigo'];
	...
	/*CLIENTES*/
	$clientesHtml='';
	$sql="select c.id, c.empresa from clientes c, partes_material pm, partes_pedido pp where 
	pm.idPedido=pp.id AND 
	...
	$smarty->display('admin/informes-material.tpl');
?>